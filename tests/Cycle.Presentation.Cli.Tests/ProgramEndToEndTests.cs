...
public sealed class ProgramEndToEndTests : IClassFixture<MsBuildFixture>, IDisposable
{
    ...
    [Fact]
    public async Task WithFormatTxt_WritesAbsolutePathsAndReturnsZero()
    {
        var (slnPath, changedFilePath, outputPath) = SetUpProject("ClassA.cs", "class A {}");
        outputPath = Path.ChangeExtension(outputPath, ".txt");

        var exitCode = await Program.Main([slnPath, outputPath, "--files", changedFilePath, "--format", "txt"]);
        ...
        lines[0].ShouldEndWith(".csproj");
    }

    [Fact]
    public async Task WithFormatTxt_NoChangedFiles_WritesEmptyFile()
    {
        var projectDir = Path.Combine(_testDir, "ProjectA");
        Directory.CreateDirectory(projectDir);
        var csprojPath = Path.Combine(projectDir, "ProjectA.csproj");
